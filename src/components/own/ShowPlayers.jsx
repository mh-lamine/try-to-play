...
export default function ShowPlayers() {
    ...
    return (
        playersList.map((player, index) => {
            ...
            const classNameCurrentPlayer = player.id === game.current_player ? 'border-4 border-black' : ''
            const classNameScorePosition = `${(player.id === 1) | (player.id === 3) ? 'right-5' : 'left-5'} bottom-1`
            return (
                <div className={`from-0% to-45%  w-1/3 border fixed ${classNameCurrentPlayer} ${classNameTopBottom} ${classNameLeftRight} ${classNameBgColor} ${classNameTextColor}`} key={index}>
